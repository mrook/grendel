{
  ...
  @lastmod($Id: MemCheck.pas,v 1.1 2003/12/12 13:19:58 ***REMOVED*** Exp $)
}
...
var
	MemCheckLogFileName: string = 'c:\Temp\MemCheck.log';	//The file memcheck will log information to
...
begin
	...
	for i:= 0 to UnitsInfo.UnitCount - 1 do
		begin
			...
			if {$IFNDEF VER140}@{$ENDIF}CurrentUnitInfo.Init = @System.System then
				NewUnitsInfoOrder.Insert(0, CurrentUnitInfoCopy)
			else
				if {$IFNDEF VER140}@{$ENDIF}CurrentUnitInfo.Init = @SysUtils.SysUtils then
					...
					{$ENDIF}
{
						if CurrentUnitInfo.Init = @Classes.Classes then
							NewUnitsInfoOrder.Insert(3, CurrentUnitInfoCopy)
						else
}						
							if {$IFNDEF VER140}@{$ENDIF}CurrentUnitInfo.Init = Pointer(PChar(@Dummy) + 356) then
...
begin
	...
	while (Block <> nil) and (B.Count < MaxNumberOfBlocks) do
		begin
			if (Block.DestructionAdress <> nil) and (Block.AllocatedSize > 5) and (Block.AllocatedSize <= MaxBlockSize) and (not IsMemFilledWithChar(pchar(Block) + SizeOf(TMemoryBlocHeader) + 4, Block.AllocatedSize - 5, CharToUseToWipeOut)) then
				B.Add(Block);
